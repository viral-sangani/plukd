...
import { EmbeddingsTable } from '@/components/admin/embeddings/embeddings-table'
import {
  useAdminEmbeddings,
  useRegenerateEmbedding,
  useBulkRegenerateEmbeddings,
  useBulkRegenerateAllEmbeddings,
  useRefreshKeyboardShortcut,
} from '@/lib/hooks/use-admin-embeddings'
...
export default function EmbeddingsPage() {
  ...
  const bulkRegenerate = useBulkRegenerateEmbeddings()
  const bulkRegenerateAll = useBulkRegenerateAllEmbeddings()

  // Handle keyboard shortcut for refresh
  useRefreshKeyboardShortcut(refetch, true)
  ...
  const handleRegenerate = useCallback(async (bookmarkId: string) => {
    ...
    try {
      await regenerateEmbedding.mutateAsync(bookmarkId)
  ...
  const handleBulkRegenerate = useCallback(async () => {
    try {
      ...
      toast.success(`Queued ${result.count} missing bookmarks for embedding regeneration`)
    } catch (err) {
  ...
  const handleBulkRegenerateAll = useCallback(async () => {
    try {
      ...
      toast.success(`Queued ${result.count} bookmarks for full embedding regeneration`)
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to queue regeneration'
      toast.error(message)
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="font-mono text-xs text-zinc-600 mt-1">
            Manage bookmarks missing embeddings
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard
          label="WITH EMB"
          value={stats.with_embeddings.toLocaleString()}
          subValue={`(${stats.coverage_percent.toFixed(1)}%)`}
        ...
        <MetricCard
          ...
          subValue="target: 95%"
          valueColor={stats.coverage_percent >= 95 ? 'text-green-500' : 'text-amber-500'}
          isLoading={isLoading}
        />
      </div>
      ...
      {isError && (
        <div className="border border-red-500/20 bg-red-500/5 p-4">
          <p className="font-mono text-sm text-red-500">
            Error loading embeddings: {error instanceof Error ? error.message : 'Unknown error'}
          </p>
          <Button
            variant="ghost"
            size="sm"
