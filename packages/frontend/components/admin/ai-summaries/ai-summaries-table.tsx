...
import type { AISummaryBookmark, AISummariesStats } from '@/lib/hooks'

interface AISummariesTableProps {
...
function getStatusColor(status: AISummaryBookmark['processing_status']): string {
  switch (status) {
    case 'completed':
...
function getStatusLabel(status: AISummaryBookmark['processing_status']): string {
  switch (status) {
    case 'completed':
      return 'Completed'
    case 'processing':
...
export function AISummariesTable({
  bookmarks,
  stats,
  pagination,
  isLoading,
  ...
  onRegenerate,
  isRegenerating,
}: AISummariesTableProps) {
  const columns = useMemo<ColumnDef<AISummaryBookmark>[]>(
    () => [
      {
        accessorKey: 'title',
        header: () => (
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="max-w-[300px]">
              ...
              >
                <span className="truncate">{truncateText(displayUrl, 50)}</span>
  ...
  if (isLoading) {
    return <AISummariesTableSkeleton />
  }
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-4">
          <span className="font-mono text-sm text-gray-400">
            ...
            {' '}bookmarks missing AI data out of{' '}
...
function AISummariesTableSkeleton() {
  return (
    <div className="space-y-4">
      {/* Stats Skeleton */}
      <div className="flex items-center justify-between px-1">
